...
var loop = function() {
    if ( ! mouse.isDown) {
        ...
        // radiansx += ball.velocity.x*frameRate*100;
        // radiansy += ball.velocity.y*frameRate*100;
        // ball.position.x = mouse.x + Math.cos(radiansx) * radiansx;
        // ball.position.y = mouse.y + Math.sin(radiansy) * radiansy;

        ball.position.x += (ball.velocity.x*frameRate*100);
        ball.position.y += (ball.velocity.y*frameRate*100);

        if (ball.position.x < 100 || ball.position.x > 300) ball.velocity.x = -ball.velocity.x;

        if (ball.position.y < 100 || ball.position.y > 300) ball.velocity.y = -ball.velocity.y;

        // ball.position.x = ball.position.x * 0.97;
        // ball.position.y = ball.position.y * 0.97;
    }
    // Handle collisions
